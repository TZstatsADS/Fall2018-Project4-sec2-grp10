...
##step 9 output the original text with error detected
```{r}
p <- "[[:punct:]|[:digit:]]|[A-Z]"
lo <- list()
error_cleaned <- list()
...
f <- function(vector){
  for(i in 1:length(vector)){
    word <- vector[i]
    logic <- rep(NA,length(vector))
    s <- unlist(strsplit(word,split=NULL))
    logic[i] <- ifelse(sum(grepl(p,s))==nchar(word),TRUE,FALSE)
  }
  return(logic) 
}
index_selected <- list()
non_error <- list()
for(i in 1:100){
  lo[[i]] <- grepl(p,error_list[[i]])
  error_cleaned[[i]] <- error_list[[i]][!lo[[i]]]
  index_selected[[i]] <- which(lo[[i]]==FALSE)
